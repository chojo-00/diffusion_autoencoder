#!/bin/bash
#SBATCH -J diffae_train_anbcond_kooaldam_processing_ver2_normalized_bitstored_minmax_foldering_png_ruleron_claheoff
#SBATCH -t 7-00:00:00
...
# Define vars
JOB_NAME="diffae_train_anbcond_kooaldam_processing_ver2_normalized_bitstored_minmax_foldering_png_ruleron_claheoff"
...
docker run --rm \
        ...
        bash -c "
            ...
            cd ${CODE_DIR} && \
            python train_anbcond.py \
                --name my_experiment_anbcond_ver1_Kooaldam_Ruler_ON_CLAHE_OFF \
                --dataset-dir \
                ...
                --wandb-project diffae_anbcond_kooaldam_processing_ver2_normalized_bitstored_minmax_foldering_png_ruleron_claheoff \
        "
...
: << 'END_COMMENT'
# --- 스모크 테스트 예시(작은 num-itr 로 빠르게 정합성 확인) ---
        bash -c "
            cd ${CODE_DIR} && \
            python train_anbcond.py \
                --name smoke_anbcond_ver1 \
                --dataset-dir \
                    "/workspace/bc_cho/0_Project/1_class3_ceph/Mydataset/bccho/kooaldam_processing_ver2_normalized_bitstored_minmax_foldering_new" \
                --image-subdir png_RulerON_CLAHE_OFF \
                --image-size 512 \
