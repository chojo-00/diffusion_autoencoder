# train_anbloss.py
"""
train.py + ANB(전후방 골격성 각도) 보조 회귀 loss.
기존 train.py 는 수정하지 않는다.

z_sem(semantic_enc 출력) 위에 작은 회귀 헤드(anb_head, network_anbloss.py)를 붙여
ANB 각도를 같이 예측하게 하고, 기존 diffusion 복원 loss 와 멀티태스크로 학습한다.
  loss = recon_loss + lambda_anb * anb_loss
...
from dataset.image_dataset_anbloss import PNGGrayDatasetANB
from diffae.runner_anbloss import RunnerANB
...
def create_training_options():
    ...
    parser.add_argument("--anb-hidden",     type=int,   default=256,        help="anb_head hidden layer 크기")
    parser.add_argument("--lambda-anb",     type=float, default=1.0,         help="anb_loss 가중치(recon_loss 대비 균형)")
...
def main(opt):
    ...
    log.info("=======================================================")
    log.info("       Diffusion Autoencoders + ANB auxiliary loss      ")
    log.info("=======================================================")
